...
from science_verification_analysis import convert_events_information, choose_trigger_type, station_channels, read_rnog_runtable, read_rnog_data, calculate_statistics_log_snr, calculate_z_score_snr, outlier_flag, find_outlier_details, print_outlier_summary
import json
import matplotlib.dates as mdates
from datetime import timezone


'''This module can be used to find expected parameter values (SNR, glitching) for RNO-G stations from a known stable time period.'''
...
def save_k_values_json(k_values_log, station_id, save_location):
    '''Save the k-values as a JSON file.'''
    k_values_filename = f"station_{station_id}_k_ref_values_snr.json"
    with open(k_values_filename, "w") as f:
        json.dump(k_values_log, f, indent=4)

    print(f"K-values saved to {k_values_filename}")
...
if __name__ == "__main__":
    ...
    if args.save_k_values:
        save_k_values_json(k_values_log_snr, station_id, save_location)
